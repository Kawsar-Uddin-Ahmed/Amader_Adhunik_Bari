...

{
  ...
  /*--------------------For voterid verification-------------------*/
   ...
   if(empty($n) || empty($ph) || empty($g) || empty($Email) || empty($pr) || empty($ser) || empty($vii) || empty($tit) || empty($fl) || empty($fcode) ||empty($bnn) || empty($zznm) || empty($pass) || empty($file_name))
   {
   	$msg = "<span style='color:red'>Field must be filled....</span>";
    return $msg;
   }
   $mailquery = "SELECT * FROM tbl_owner WHERE email='$Email' LIMIT 1";
   $mailquer = $this->db->select($mailquery);
   $voterquery = "SELECT * FROM tbl_owner WHERE voterid='$vii' LIMIT 1";
   $voterquer = $this->db->select($voterquery);
    
   $flatcodequery = "SELECT * FROM tbl_owner WHERE flat_code='$fcode' LIMIT 1";
   $flatcodeq = $this->db->select($flatcodequery);

   $numberquery = "SELECT * FROM tbl_owner WHERE number='$ph' LIMIT 1";
   $numeberquer = $this->db->select($numberquery);

   /*$flatquery = "SELECT * FROM tbl_user WHERE flat_no='$fl'and (title ='2' and status=0) LIMIT 1";*//*status=0 is used if the owner send the renter in history*/
   ...
   elseif($voterquer != false)
   {
    ...
    elseif($flatcodeq != false)//this for owner not repeat two code
   {
    $msg = "<span style='color:red'>This flat code already has an owner..</span>";
    return $msg;
   }
    elseif($numeberquer != false)//this for owner not repeat two code
   {
    $msg = "<span style='color:red'>Phone number already remains...</span>";
    return $msg;
   }
   /*elseif($flatquer != false)
   {
    $msg = "<span style='color:red'>This flat is already filled..</span>";
    return $msg;
   }*/
    ...
    if(in_array($file_extensions,$permitted) === false)
     {
      ...
      return $msg;
          ...
          /*$inserting = "INSERT INTO tbl_user (name,gender,voterid,proff,company,number,email,title,pass,bname,flat_no,flat_code,state,image) VALUES('$n','$g','$vii','$pr','$ser','$ph','$Email','$tit','$pass',$bnn','$fl','$fcode','$stat','$uploaded_image')";*/
          $inserting = "INSERT INTO tbl_owner(name,gender,voterid,proff,company,number,email,title,pass,bname,flat_no,flat_code,zone_name,state,image) VALUES('$n','$g','$vii','$pr','$ser','$ph','$Email','$tit','$pass','$bnn','$fl','$fcode','$zznm','$stat','$uploaded_image')";
   	        $inserted_row=$this->db->insert($inserting);
            if ($inserted_row) 
             {
                $msg= "<span style='color:white'>Owner Registration completed succesfully </span>";
                return $msg;
            }
            else 
            {
                $msg= "<span style='color:red'>Owner Registration Inserted Failed</span>";
                return $msg;
            }          
   }
 }
