...
<?php
//$pp = new payment();
$mail = Session::get('email'); //this is renter email session
?>
<!-------------for showing the payment renter send to the admin to the owner to collect the money---------->
<?php
//$query = "SELECT tbl_payment.*,month.mon FROM tbl_payment INNER JOIN month ON tbl_payment.month = month.monthid WHERE remail = '$mail' and status = 1";
$query = "SELECT * FROM tbl_payment WHERE remail='$mail' AND status = 1";
$gg = $db->select($query);
if($gg)
{
     $i = 0;
